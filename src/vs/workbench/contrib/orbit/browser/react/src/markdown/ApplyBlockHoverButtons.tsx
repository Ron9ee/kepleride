...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: { tooltipName: string, color?: IndicatorColor, position?: PlacesType, offset?: number }) => ({
	'data-tooltip-id': color === 'orange' ? `kepler-tooltip-orange` : color === 'green' ? 'kepler-tooltip-green' : 'kepler-tooltip',
...
const ApplyButtonsForEdit = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		if (!applyDonePromise) {
			notificationService.info(`Kepler Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`)
		}
		...
		applyDonePromise?.catch(e => {
			...
			notificationService.info(`Kepler Error: There was a problem running Apply: ${e}.`)
