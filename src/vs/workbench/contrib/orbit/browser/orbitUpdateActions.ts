...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Kepler, please download the latest version! [Kepler Editor](https://github.com/ashish200729/Keplereditor-binaries/releases/latest)!'
		...
		if (res.action === 'reinstall') {
			primary.push({
				...
				run: () => {
					...
					window.open('https://github.com/ashish200729/Keplereditor-binaries/releases/latest')
				}
			})
		}
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Kepler Error: There was an error checking for updates. If this persists, please get in touch or reinstall Kepler [here](https://github.com/ashish200729/Keplereditor-binaries/releases/latest)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	console.log('[Kepler Update] Starting check, explicit:', explicit)
	metricsService.capture(`Kepler Update ${metricsTag}: Checking...`, {})
	const res = await voidUpdateService.check(explicit)
	console.log('[Kepler Update] Check result:', res)

	if (!res) {
		const notifController = notifyErrChecking(notifService);
		metricsService.capture(`Kepler Update ${metricsTag}: Error`, { res })
		return notifController
	}
	else {
		if (res.message) {
			const notifController = notifyUpdate(res, notifService, updateService)
			metricsService.capture(`Kepler Update ${metricsTag}: Yes`, { res })
			return notifController
		}
		else {
			console.log('[Kepler Update] No message to show - update check returned null message')
			metricsService.capture(`Kepler Update ${metricsTag}: No`, { res })
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('keplerCheckUpdate', 'Kepler: Check for Updates'),
		});
	}
