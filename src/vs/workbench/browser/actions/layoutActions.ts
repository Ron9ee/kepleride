...
MenuRegistry.appendMenuItems([{
	...
	command: {
		id: ToggleSidebarPositionAction.ID,
		title: localize('move second sidebar left', "Move Kepler Side Bar Left")
		},
	...
	command: {
		id: ToggleSidebarPositionAction.ID,
		title: localize('move second sidebar right', "Move Kepler Side Bar Right")
		},
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "Kepler Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!(isViewSolo && currentLocation === ViewContainerLocation.AuxiliaryBar)) {
			items.push({
				...
				label: localize('moveFocusedView.newContainerInSidePanel', "New Kepler Side Bar Entry")
			});
		}
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "Kepler Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "Kepler Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),
...
registerAction2(class CustomizeLayoutAction extends Action2 {
	...
	getItems(contextKeyService: IContextKeyService, keybindingService: IKeybindingService): QuickPickItem[] {
		...
		// Both modes show: Primary Side Bar, Kepler Side Bar (Chat), Panel
