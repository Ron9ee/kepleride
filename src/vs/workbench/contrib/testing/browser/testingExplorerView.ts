...
export class TestingExplorerView extends ViewPane {
	 ...
	 * bitset is collected.
	 */
	public getTreeIncludeExclude(profileKeplerset: ITestRunProfile | TestRunProfileBitset, withinItems?: InternalTestItem[], filterToType: 'visible' | 'selected' = 'visible') {
		...
		const runnableWithProfileKeplerset = new Map<InternalTestItem, boolean>();
		const isRunnableWithProfileKeplerset = (item: InternalTestItem) => {
			let value = runnableWithProfileKeplerset.get(item);
			if (value === undefined) {
				value = typeof profileKeplerset === 'number'
					? !!this.testProfileService.getDefaultProfileForTest(profileKeplerset, item)
					: canUseProfileWithTest(profileKeplerset, item);
				runnableWithProfileKeplerset.set(item, value);
			}
			return value;
		};
		...
		const attempt = (element: TestExplorerTreeElement, alreadyIncluded: boolean) => {
			...
			const visibleRunnableChildren = inTree.children.filter(
				c => c.visible
					...
					&& isRunnableWithProfileKeplerset(c.element.test),
			).length;
			...
			if (
				...
				&& isRunnableWithProfileKeplerset(element.test)
		...
		for (const root of withinItems || this.testService.collection.rootItems) {
			...
			if (typeof profileKeplerset === 'object' && !canUseProfileWithTest(profileKeplerset, root)) {
				continue;
			}
