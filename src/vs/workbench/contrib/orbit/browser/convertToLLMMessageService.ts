...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .Keplerrules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	// Read .Keplerrules files from workspace folders
	private _getKeplerRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let KeplerRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.Keplerrules')
				const { model } = this.voidModelService.getModel(uri)
				if (!model) continue
				KeplerRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return KeplerRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .Keplerrules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.voidSettingsService.state.globalSettings.aiInstructions;
		const KeplerRulesFileContent = this._getKeplerRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (KeplerRulesFileContent) ans.push(KeplerRulesFileContent)
		return ans.join('\n\n')
	}
