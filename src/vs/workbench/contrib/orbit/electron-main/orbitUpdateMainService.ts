...
export class VoidMainUpdateService extends Disposable implements IVoidUpdateService {
	...
	async check(explicit: boolean): Promise<VoidCheckUpdateRespose> {
		...
		if (isDevMode) {
			console.log('[Kepler Update] Running in dev mode - checking GitHub releases anyway for testing')
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Kepler to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<VoidCheckUpdateRespose> {
		try {
			console.log('[Kepler Update] Fetching latest release from GitHub binaries repo...')
			const response = await fetch('https://api.github.com/repos/ashish200729/Keplereditor-binaries/releases/latest', {
				headers: {
					'Accept': 'application/vnd.github.v3+json'
				}
			});

			console.log('[Kepler Update] GitHub API response status:', response.status)
			const data = await response.json();
			const version = data.tag_name;
			console.log('[Kepler Update] Latest GitHub release tag:', version)

			const myVersion = this._productService.version
			const latestVersion = version

			console.log('[Kepler Update] Current version:', myVersion, '| Latest version:', latestVersion)
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Kepler is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Kepler is up-to-date!'
					}
				}
				else {
			...
			console.error('[Kepler Update] Error fetching GitHub release:', e)
			if (explicit) {
				return {
